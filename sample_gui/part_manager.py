...
def add_item():
    if part_text.get() == '' \
            or customer_text.get() == '' \
            or retailer_text.get() == '' \
            or price_text.get() == '':
...
# Part
part_text = StringVar()
part_label = Label(app, text='Part Name', font=('bold', 14), padx=20)
...
# Parts List (Listbox)
parts_list = Listbox(app, height=9, width=60)
parts_list.grid(row=3, column=0, columnspan=3, rowspan=6, padx=20)

# Create scorllbar
scrollbar = Scrollbar(app)
scrollbar.grid(row=3, column=3, rowspan=6, sticky=N + S + W)
